...
class PolicyHook(Hook):
    ...
    def _acts_info(self, acts):
        ...
        return [str(int(sum(act0).item())) + "/" + str(self.action_dims[0]),
                str(int(sum(act1).item())) + "/" + str(self.action_dims[1]),
                str(int(sum(act2).item())) + "/" + str(self.action_dims[2])]
    ...
    def _reset_critic(self, runner):
        (state, adj) = self._get_observation(runner)
