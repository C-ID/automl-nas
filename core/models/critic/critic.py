...
def WattsStrogatz(nodeNum):
    g = nx.random_graphs.watts_strogatz_graph(nodeNum, 4, 0.15)
    g = g.to_directed()
    ...
    return np.ones((nodeNum, nodeNum), dtype=np.int32).tolist()

@CRITIC.register("CriticTopo")
class CriticTopo(nn.Module):
    """
        Critic network.
    ...
    def __init__(
        ...
        super(CriticTopo, self).__init__()
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(3, backbone.C_in, kernel_size=3, padding=1, bias=False),
