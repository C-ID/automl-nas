...
@CRITIC.register("ReductCritic")
class ReductCritic(nn.Module):
    """
        Critic network.
    ...
    def __init__(
        ...
        super(ReductCritic, self).__init__()
        ...
        self.stem = nn.Sequential(
            nn.Conv2d(3, backbone.C_in//2, kernel_size=1, bias=False),
            nn.BatchNorm2d(backbone.C_in//2),
            nn.ReLU(inplace=True),
            nn.Conv2d(backbone.C_in//2, backbone.C_in, kernel_size=1, stride=2, bias=False),
            nn.BatchNorm2d(backbone.C_in),
            nn.ReLU(inplace=True),
            nn.Conv2d(backbone.C_in, backbone.C_in, kernel_size=3, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(backbone.C_in),
            nn.ReLU(inplace=True),
            nn.Conv2d(backbone.C_in, backbone.C_in, kernel_size=1, stride=2, bias=False),
            nn.BatchNorm2d(backbone.C_in),
            nn.ReLU(inplace=True),
            nn.Conv2d(backbone.C_in, backbone.C_in, kernel_size=3, stride=2, padding=1, bias=False),
            nn.BatchNorm2d(backbone.C_in),
            nn.ReLU(inplace=True),
            nn.AvgPool2d(3, stride=2, padding=1, count_include_pad=False),
            nn.Conv2d(backbone.C_in, backbone.C_in, kernel_size=1, stride=1, padding=1, bias=False),
            nn.BatchNorm2d(backbone.C_in),
            nn.ReLU(inplace=True),
            nn.AvgPool2d(3, stride=2, padding=1, count_include_pad=False),
            nn.Conv2d(backbone.C_in, backbone.C_in, kernel_size=3, stride=1, padding=1, bias=False),
    ...
    def _build_arch(self, backbone, reduction_bn, neck, reduction_nh, cls_head):
        ...
        return nn.ModuleDict({
            "backbone": self._build_upper_module(backbone),
            "bridge1": self._build_bridge_block(reduction_bn),
            "neck": self._build_upper_module(neck),
            "bridge2": self._build_bridge_block(reduction_nh),
            "head": self._build_upper_module(cls_head)
        })
