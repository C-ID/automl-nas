...
topo_nums = [8, 34, 8]                         # Three topo graph, construct whole picture
state_space = [int(8*7/2+34*33/2+8*7/2), 5]     #  i.e. dims = [[topo[0]*(topo[0]-1))/2 , 5],
                                              #               [topo[1]*(topo[1]-1))/2 , 5],
                                              #               [topo[2]*(topo[2]-1))/2 , 5]
cls_num=1000
Space = dict(
...
c_optim = dict(
    optimizer=dict(type='SGD', lr=0.1, momentum=0.95, weight_decay=0.0005),
    optimizer_config=dict(grad_clip=dict(max_norm=35, norm_type=2)),
)
a_optim = dict(
    optimizer=dict(type='Adam', lr=0.2, betas=(0.5,0.999), weight_decay=0.0003),
)
...
work_dir = 'ImageNet/stage_noAug_n34'
