...
state_space = [int(20*19/2+15*14/2+10*9/2), 5]   #  i.e. dims = cat([[topo[0]*(topo[0]-1))/2 , 5],
                                                 #                   [topo[1]*(topo[1]-1))/2 , 5],
                                                 #                   [topo[2]*(topo[2]-1))/2 , 5]], dim=0)
cls_num=1000
Space = dict(
...
c_optim = dict(
    optimizer=dict(type='SGD', lr=0.3, momentum=0.9, weight_decay=0.0003, nesterov=True),
...
work_dir = 'ImageNet/critic_rx5stem_c128b20n15h10_0.3C_0.05A_250RL'
