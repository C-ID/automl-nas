...
@PIPELINES.register("AutoAugment")
...
@PIPELINES.register("RandAugment")
...
class Translate(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            img_translated = tools.imtranslate(
                img,
                offset,
                direction=self.direction,
                border_value=self.pad_val,
...
@PIPELINES.register("AutoContrast")
class AutoContrast(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_contrasted = tools.auto_contrast(img)
...
class Invert(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_inverted = tools.iminvert(img)
...
class Equalize(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_equalized = tools.imequalize(img)
...
class Solarize(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_solarized = tools.solarize(img, thr=self.thr)
...
@PIPELINES.register("SolarizeAdd")
...
class Posterize(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_posterized = tools.posterize(img, bits=self.bits)
...
class Contrast(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_contrasted = tools.adjust_contrast(img, factor=1 + magnitude)
            results[key] = img_contrasted.astype(img.dtype)
        return results
...
@PIPELINES.register("ColorTransform")
class ColorTransform(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_color_adjusted = tools.adjust_color(img, alpha=1 + magnitude)
...
class Brightness(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_brightened = tools.adjust_brightness(img, factor=1 + magnitude)
...
class Sharpness(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_sharpened = tools.adjust_sharpness(img, factor=1 + magnitude)
...
class Cutout(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            img = results[key]
            img_cutout = tools.cutout(img, self.shape, pad_val=self.pad_val)
