...
class RandomResizedCrop(object):
    ...
    def __call__(self, results):
        for key in results.get('img_fields', ['img']):
            ...
            img = tools.imcrop(img, bboxes=np.array([xmin, ymin, xmax, ymax]))
            results[key] = tools.imresize(
                img,
                tuple(self.size[::-1]),
...
@PIPELINES.register("RandomGrayscale")
...
@PIPELINES.register("RandomErasing")
...
class CenterCrop(object):
    ...
    def __call__(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            # crop the image
            img = tools.imcrop(img, bboxes=np.array([x1, y1, x2, y2]))
...
class Normalize(object):
    ...
    def __call__(self, results):
        for key in results.get('img_fields', ['img']):
            results[key] = tools.imnormalize(results[key], self.mean, self.std,
                                            self.to_rgb)
...
@PIPELINES.register("ColorJitter")
...
class Lighting(object):
    ...
    def __call__(self, results):
        for key in results.get('img_fields', ['img']):
            img = results[key]
            results[key] = tools.adjust_lighting(
                img,
                self.eigval,
                self.eigvec,
