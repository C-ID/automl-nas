...
def load_checkpoint(model,
    ...
    if checkpoint.get('actor', False):
        model['actor'].module.load_state_dict(checkpoint['actor'])
    elif checkpoint.get('actor_optimizer', False):
        model['actor_optimizer'].load_state_dict(checkpoint['actor_optimizer'])
    elif checkpoint.get('critic', False):
        model['critic'].module.load_state_dict(checkpoint['critic'])
    elif checkpoint.get('critic_optimizer', False):
        model['critic_optimizer'].module.load_state_dict(checkpoint['critic_optimizer'])
    elif checkpoint.get('last_routing', False):
        model['critic'].set_last_routing(checkpoint['last_routing'])
    return checkpoint
...
def save_checkpoint(model, filename, optimizer=None, meta=None):
    ...
    tools.mkdir_or_exist(osp.dirname(filename))
    # if hasattr(model, 'module'):
    #     model = model.module
    #
    # checkpoint = {
