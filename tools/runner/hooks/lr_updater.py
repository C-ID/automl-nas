...
class LrUpdaterHook(Hook):
    ...
    def _set_lr(self, lr_groups):
        if isinstance(self.optim, dict):
            for k, optim in self.optim.items():
                for param_group, lr in zip(optim.param_groups, lr_groups[k]):
                    param_group['lr'] = lr
        else:
            for param_group, lr in zip(self.optim.param_groups,
                                       lr_groups):
                param_group['lr'] = lr
    ...
    def before_run(self, runner):
        ...
        if isinstance(self.optim, dict):
            ...
            for group in self.optim.param_groups:
                group.setdefault('initial_lr', group['lr'])
            self.base_lr = [
                group['initial_lr'] for group in self.optim.param_groups
            ]
...
class StepLrUpdaterHook(LrUpdaterHook):
    ...
    def __init__(self, step, gamma=0.1, min_lr=None, **kwargs):
        if isinstance(step, list):
            assert tools.is_list_of(step, int)
...
class OneCycleLrUpdaterHook(LrUpdaterHook):
    ...
    def before_run(self, runner):
        ...
        if isinstance(self.optim, dict):
            ...
            k = type(self.optim).__name__
            _max_lr = format_param(k, self.optim, self._max_lr)
            self.base_lr = [lr / self.div_factor for lr in _max_lr]
            for group, lr in zip(self.optim.param_groups, self.base_lr):
                group.setdefault('initial_lr', lr)
